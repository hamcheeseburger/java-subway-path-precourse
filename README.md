# 기능 목록
- [x] 초기 지하철 경로를 설정한다.
- [x] 거리를 기준으로 다익스트라 그래프를 만든다.
- [x] 시간을 기준으로 다익스트라 그래프를 만든다.
- [x] 사용자는 경로조회(1) 또는 종료(Q)를 할 수 있다.
- [x] 1 또는 Q가 아닌 값은 존재하지 않는다.
- [x] 경로조회를 선택했을 때 최단거리(1), 최소시간(2), 돌아가기(B)의 입력을 받을 수 있다.
- [ ] 출발역과 도착역은 문자이어야 한다.
- [ ] 출발역과 도착역은 같을 수 없다.
- [ ] 출발역과 도착역은 지하철역으로 존재하는 값 이어야한다.
